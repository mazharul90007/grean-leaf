...
const getDoctorsAllSchedules = async (filters, options) => {
...
const deleteDoctorSchedule = async (scheduleId, user) => {
    const doctorData = await prisma.doctor.findUniqueOrThrow({
        where: {
            email: user?.email,
        },
    ...
    if (isBookedSchedule) {
        throw new ApiError(status.BAD_REQUEST, "Already Booked Schedule can not be deleted");
    }
